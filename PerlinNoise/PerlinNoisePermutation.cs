...
{
    public class PerlinNoisePermutation
    {
        readonly int[] p;                                                    // Doubled permutation to avoid overflow

        readonly int[] permutation = { 151,160,137,91,90,15,					// Hash lookup table as defined by Ken Perlin.  This is a randomly
        ...
        public double NextOctave(double x, double y, double z, int octaves, double persistence)
        {
            double total = 0;
            double frequency = 1;
            double amplitude = 1;
            ...
            {
                total += Next(x * frequency, y * frequency, z * frequency) * amplitude;
